//+------------------------------------------------------------------+
//| Robo_Recuperado_VALIDATION_v109_ROBOTNAME.mq5                              |
//| Forex IA v108 - CSV por nome do robo             |
//| Não abre ordens. Apenas gera CSV para validação.                  |
//+------------------------------------------------------------------+
#property strict
#property version   "109.00"
#property description "Forex IA Validation v109: CSV por nome do robo, SafeName corrigido, FileFlush/FileClose."
...
void WriteValidationCSV(string signal, datetime barTime, double entry, double ema, double rsi, double macd, double macdSignal, double roc){
   ...
   if(h == INVALID_HANDLE){ Print("FOREX IA v109 ERRO CSV: ", GetLastError(), " arquivo=", file); return; }
...
void WriteAuditCSV(datetime barTime, int sh, double open, double high, double low, double close, double entry,
   ...
   bool exists = FileIsExist(file, FILE_COMMON);
   int h = FileOpen(file, FILE_READ|FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI, ';');
   if(h == INVALID_HANDLE){ Print("FOREX IA v109 ERRO AUDIT: ", GetLastError(), " arquivo=", file); return; }
   FileSeek(h, 0, SEEK_END);
   if(!exists || FileTell(h) == 0){
...
int OnInit(){
   if(Apagar_CSV_Ao_Iniciar){
      ...
      Print("FOREX IA v109: CSVs antigos apagados.");
   }
   Print("FOREX IA v109 iniciado. Robo=", RobotName, " RunId=", Validation_RunId, " CSV=", CsvFile(), " Symbol=", _Symbol, " TF=", TF());
...
void OnTick(){
   ...
   if(signal=="CALL" || signal=="PUT"){
      ...
      Print("FOREX IA v109 CSV: ", signal, " ", TimeToString(barTime,TIME_DATE|TIME_MINUTES), " entry=", DoubleToString(entry,_Digits), " close=", DoubleToString(close,_Digits), " ema=", DoubleToString(ema,8), " rsi=", DoubleToString(rsi,4), " hist=", DoubleToString(hist,8));
