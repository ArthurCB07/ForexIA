...
type Page='dashboard'|'billing'|'import'|'datasets'|'viewer'|'builder'|'robots'|'compare'|'lab'|'optimizer'|'validation'|'voice'|'ranking'|'setup';type DS={id:string;pair:string;timeframe:string;count:number;first:string;last:string};
function Sidebar({page,setPage}:any){const items:any[]=[
 ['dashboard',Home,'Dashboard'],
 ['billing',Settings,'Créditos'],
...
function BillingPage(){
 const[info,setInfo]=useState<any>(null),[name,setName]=useState(''),[email,setEmail]=useState(''),[amount,setAmount]=useState(50),[msg,setMsg]=useState('');
 async function load(){try{const r=await api('/api/billing/status');setInfo(r);setName(r.user?.name||'');setEmail(r.user?.email||'')}catch(e:any){setMsg(String(e.message||e))}}
 useEffect(()=>{load()},[]);
 async function register(){setMsg('');try{const r=await api('/api/billing/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,email})});setInfo(r);setMsg('Conta selecionada/criada com sucesso.')}catch(e:any){setMsg(String(e.message||e))}}
 async function topup(v?:number){setMsg('');try{const r=await api('/api/billing/topup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({amount:v||amount})});setInfo(r);setMsg('Créditos adicionados.')}catch(e:any){setMsg(String(e.message||e))}}
 const pr=info?.pricing||{};
 return <section><h1>Créditos e Conta</h1>
  <div className="panel"><h2>Conta ativa</h2>{info?<div className="cards billingCards"><div className="card"><span>Usuário</span><b>{info.user.name}</b></div><div className="card"><span>E-mail</span><b>{info.user.email}</b></div><div className="card"><span>Saldo</span><b>{money(info.user.balance)}</b></div></div>:<p>Carregando...</p>}
   <div className="grid"><label>Nome<input value={name} onChange={e=>setName(e.target.value)} placeholder="Nome do usuário"/></label><label>E-mail<input value={email} onChange={e=>setEmail(e.target.value)} placeholder="email@empresa.com"/></label></div><button onClick={register}>Entrar / criar conta</button>{msg&&<p className="warn">{msg}</p>}
  </div>
  <div className="panel"><h2>Adicionar créditos</h2><p>Versão local de demonstração: a recarga apenas registra saldo no banco local.</p><div className="presetBtns"><button onClick={()=>topup(20)}>R$ 20</button><button onClick={()=>topup(50)}>R$ 50</button><button onClick={()=>topup(100)}>R$ 100</button><button onClick={()=>topup(200)}>R$ 200</button></div><label>Outro valor<input type="number" value={amount} onChange={e=>setAmount(+e.target.value)}/></label><button onClick={()=>topup()}>Adicionar valor</button></div>
  ...
  <div className="panel"><h2>Extrato</h2><table><thead><tr><th>Data</th><th>Descrição</th><th>Valor</th><th>Saldo</th></tr></thead><tbody>{(info?.ledger||[]).map((x:any)=><tr key={x.id}><td>{new Date(x.createdAt).toLocaleString('pt-BR')}</td><td>{x.description}{x.indicators?` • ${x.indicators} indicador(es)`:''}</td><td>{money(x.amount)}</td><td>{money(x.balanceAfter)}</td></tr>)}</tbody></table></div>
 </section>
}
...
function VoiceAgent({apply}:any){
 const[text,setText]=useState(''),[parsed,setParsed]=useState<any>(null),[listening,setListening]=useState(false),[creating,setCreating]=useState(false),[created,setCreated]=useState<any>(null),[monitorMsg,setMonitorMsg]=useState('');
 const[robotName,setRobotName]=useState('Robô Voz'),[whatsStatus,setWhatsStatus]=useState<any>(null);
 ...
 useEffect(()=>{api('/api/whatsapp/status').then(setWhatsStatus).catch(()=>{})},[]);
 function start(){const SR=(window as any).SpeechRecognition||(window as any).webkitSpeechRecognition;if(!SR){alert('Seu navegador não liberou reconhecimento de voz. Use Chrome/Edge.');return}const rec=new SR();rec.lang='pt-BR';rec.continuous=false;rec.interimResults=false;rec.onresult=(e:any)=>{const tx=e.results[0][0].transcript;setText(tx);setParsed(parseVoiceToStrategy(tx));setCreated(null);setRobotName('Robô Voz '+new Date().toLocaleDateString('pt-BR'));setMonitorMsg('');setListening(false)};rec.onerror=()=>setListening(false);rec.onend=()=>setListening(false);recRef.current=rec;setListening(true);rec.start()}
 function manual(){const p=parseVoiceToStrategy(text);setParsed(p);setCreated(null);if(robotName==='Robô Voz')setRobotName('Robô Voz '+new Date().toLocaleDateString('pt-BR'));setMonitorMsg('')}
 ...
 async function startMonitor(){
   ...
   setCreating(true);setMonitorMsg('');
   try{
     const r=await api('/api/voice-monitor/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({robotId:created.id,robotName:created?.name||robotName,mode:executionMode,whatsapp,sendEntries,sendCloses,dailyReport})});
...
function BillingMini({indicatorCount=0}:any){
 ...
 const saldo=Number(info?.user?.balance||0);
 return <div className="billingMini">
  <div className="billingHead"><div><b>Créditos e custo estimado</b><span>{n} indicador(es) no robô</span></div><strong>Saldo: {money(saldo)}</strong></div>
  <div className="billingCostGrid">
   <div><span>Salvar/criar robô</span><b>{money(create)}</b></div>
   <div><span>Backtest</span><b>{money(backtest)}</b></div>
   <div><span>Otimização</span><b>{money(opt)}</b></div>
  </div>
  {saldo<create&&<p className="warn">Saldo insuficiente para salvar este robô. Adicione créditos antes de continuar.</p>}
  <div className="quickCredit"><button onClick={()=>topup(10)}>+ R$ 10</button><button onClick={()=>topup(20)}>+ R$ 20</button><button onClick={()=>topup(50)}>+ R$ 50</button><label>Outro valor<input type="number" value={amount} onChange={e=>setAmount(+e.target.value)}/></label><button onClick={()=>topup()}>Adicionar crédito</button></div>
...
function OptimizerPage({datasets,setPage,setSelected,setVoiceConfig,setSelectedRobot}:any){
 ...
 const[population,setPopulation]=useState(24),[generations,setGenerations]=useState(5),[minTrades,setMinTrades]=useState(50),[maxDrawdown,setMaxDrawdown]=useState(120),[running,setRunning]=useState(false),[result,setResult]=useState<any>(null),[err,setErr]=useState(''),[startedAt,setStartedAt]=useState<number>(0),[now,setNow]=useState<number>(Date.now());
 ...
 const speedGuess=55; // testes/minuto típico no notebook do usuário após v115
 const etaSec=Math.max(5,Math.round((plannedTests/speedGuess)*60));
 const elapsedSec=running?Math.max(0,Math.round((now-startedAt)/1000)):0;
 const progress=running?Math.min(96,Math.round((elapsedSec/Math.max(etaSec,1))*100)):0;
 const remainSec=running?Math.max(0,etaSec-elapsedSec):etaSec;
 ...
 async function run(){
  ...
  setStartedAt(Date.now());setNow(Date.now());setRunning(true);
  try{const r=await api('/api/optimizer/genetic',{method:'POST',headers:{'Content-Type':'application/json'},timeoutMs:900000,body:JSON.stringify({datasetId,robotId,voiceStrategy:(robot?.json?.voiceStrategy||robot?.voiceStrategy),filters,population,generations,minTrades,maxDrawdown,expiration:1,payout:.85,stake:1,initial:100})});setResult(r)}catch(e:any){const m=String(e?.message||e);setErr(m.includes('abort')||m.includes('aborted')?'A otimização demorou demais. Reduza População/Gerações, use o preset Balanceado ou reduza o período.':'Erro na otimização: '+m)}finally{setRunning(false)}
 }
 ...
 return <section>
  <LoadingOverlay show={running} text="🧬 Otimizador genético em execução. Testando combinações do robô selecionado..." onForceClose={()=>setRunning(false)}/>
  {running&&<div className="optimizerFloat"><h3>🧬 Otimização em andamento</h3><div className="bar"><i style={{width:progress+'%'}}/></div><div className="optStats"><span>Progresso estimado<br/><b>{progress}%</b></span><span>Decorrido<br/><b>{fmt(elapsedSec)}</b></span><span>Restante estimado<br/><b>{fmt(remainSec)}</b></span><span>Testes previstos<br/><b>{br(plannedTests)}</b></span></div><p>Não clique novamente. O tempo é estimado pela velocidade média local.</p></div>}
  ...
  <div className="panel"><h2>Configuração</h2><p>Use a plataforma para testar variações rapidamente. Depois valide no MT5 somente a melhor configuração. Padrão recomendado: <b>Balanceado</b>.</p>
   ...
   <div className="optimizerHint"><b>{profile}</b> • Testes previstos: <b>{br(plannedTests)}</b> • Tempo estimado: <b>{fmt(etaSec)}</b> • Custo da otimização: <b>{money(((robot?.json?.voiceStrategy?.indicators?.length||robot?.voiceStrategy?.indicators?.length||1)*0.50))}</b>. {warning&&<span className="warn"> {warning}</span>}</div>
   ...
   <button disabled={running||!robotId||!datasetId} onClick={run}>{running?'Otimizando...':'Executar otimização genética'}</button>{err&&<p className="warn">{err}</p>}
  </div>
...
function Setup(){
 const[cfg,setCfg]=useState<any>({provider:'evolution',evolutionUrl:'http://localhost:8080',instanceName:'forex-ia',globalKey:'',instanceToken:''});
 const[msg,setMsg]=useState(''),[qr,setQr]=useState(''),[outbox,setOutbox]=useState<any[]>([]),[loading,setLoading]=useState(false);
 async function load(){try{const r=await api('/api/whatsapp/config');setCfg((x:any)=>({...x,...r.config,provider:'evolution'}));setOutbox(r.outbox||[]);setQr(r.config?.lastQr||'')}catch(e:any){setMsg(e.message)}}
 useEffect(()=>{load()},[]);
 async function save(){setLoading(true);setMsg('');try{const r=await api('/api/whatsapp/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...cfg,provider:'evolution'})});setCfg((x:any)=>({...x,...r.config,provider:'evolution'}));setMsg('Configuração salva.')}catch(e:any){setMsg('Erro: '+e.message)}finally{setLoading(false)}}
 async function createInstance(){setLoading(true);setMsg('');try{const r=await api('/api/whatsapp/evolution/create-instance',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...cfg,provider:'evolution'})});setQr(r.qrcode||'');setMsg(r.tokenCreated?'Instância criada. Token salvo. Escaneie o QR Code.':'Instância criada. Escaneie o QR Code.');load()}catch(e:any){setMsg('Erro ao criar instância: '+e.message)}finally{setLoading(false)}}
 async function connect(){setLoading(true);setMsg('');try{const r=await api('/api/whatsapp/evolution/connect');setQr(r.qrcode||'');setMsg('QR/pareamento gerado. Escaneie no WhatsApp.');load()}catch(e:any){setMsg('Erro ao conectar: '+e.message)}finally{setLoading(false)}}
 async function status(){setLoading(true);setMsg('');try{const r=await api('/api/whatsapp/evolution/status');setMsg('Status da instância: '+(r.state||'desconhecido'));load()}catch(e:any){setMsg('Erro ao consultar status: '+e.message)}finally{setLoading(false)}}
 return <section><h1>Instalação</h1><div className="panel"><h2>WhatsApp Evolution API</h2><p>Configure a Evolution API para o botão <b>Enviar alerta teste</b> chegar no celular. Se não configurar, o sistema apenas salva a mensagem em <code>data/whatsapp_outbox.json</code>.</p><div className="grid"><label>URL da Evolution API<input value={cfg.evolutionUrl||''} onChange={e=>setCfg({...cfg,evolutionUrl:e.target.value})} placeholder="http://localhost:8080"/></label><label>Nome da instância<input value={cfg.instanceName||''} onChange={e=>setCfg({...cfg,instanceName:e.target.value})} placeholder="forex-ia"/></label><label>API Key global<input value={cfg.globalKey||''} onChange={e=>setCfg({...cfg,globalKey:e.target.value})} placeholder="AUTHENTICATION_API_KEY"/></label><label>Token da instância<input value={cfg.instanceToken||''} onChange={e=>setCfg({...cfg,instanceToken:e.target.value})} placeholder="preenche automaticamente ao criar instância"/></label></div><div className="actionsRow"><button disabled={loading} onClick={save}>Salvar configuração</button><button disabled={loading} onClick={createInstance}>Criar instância + QR</button><button disabled={loading} onClick={connect}>Gerar QR novamente</button><button disabled={loading} onClick={status}>Verificar status</button></div>{msg&&<p className={msg.startsWith('Erro')?'warn':'ok'}>{msg}</p>}{qr&&<div className="parsed"><h3>QR / Código de pareamento</h3>{String(qr).startsWith('data:image')?<img src={qr} style={{maxWidth:260,background:'#fff',padding:10,borderRadius:12}}/>:<pre>{qr}</pre>}<p>Abra o WhatsApp no celular → Aparelhos conectados → Conectar aparelho.</p></div>}<h2>Como subir a Evolution API localmente</h2><pre>docker compose up -d</pre><p>Incluí um <b>docker-compose.evolution.yml</b> no projeto. Depois configure a URL como <b>http://localhost:8080</b> e use a chave definida no arquivo.</p><h2>MT5</h2><p>WebRequest para <b>http://127.0.0.1:3001</b>.</p>{outbox?.length>0&&<><h2>Últimas mensagens</h2><div className="optimizerTable"><div className="head">Status</div><div className="head">Destino</div><div className="head">Tipo</div><div className="head">Erro/retorno</div>{outbox.slice(0,8).map((x:any)=><React.Fragment key={x.id}><div>{x.status}</div><div>{x.to}</div><div>{x.type}</div><div>{x.delivery?.error||x.delivery?.status||''}</div></React.Fragment>)}</div></>}</div></section>}
