...
const app=express(),PORT=process.env.API_PORT||3001,VERSION='106.0.0';
...
const save=d=>{d.version=VERSION;fs.writeFileSync(DB,JSON.stringify(d,null,2),'utf8')};
...
app.post('/api/mt5/candles',(req,res)=>{try{const{symbol,timeframe,candles,source}=req.body||{};if(!symbol||!timeframe||!Array.isArray(candles))return res.status(400).json({ok:false,error:'Formato inválido',version:VERSION});const rec=upsert(symbol,timeframe,candles,source||'MT5 Bridge v25');res.json({ok:true,version:VERSION,dataset:rec,received:candles.length,total:rec.count})}catch(e){res.status(500).json({ok:false,error:e.message,version:VERSION})}});
...
app.post('/api/backtest',(req,res)=>{const ds=loadSet(req.body.datasetId);if(!ds)return res.status(404).json({erro:'Dataset não encontrado'});if(ds.candles.length<80)return res.status(400).json({erro:'Poucos candles'});const result=backtest(ds.candles,req.body),d=db(),rec={id:uuidv4(),datasetId:req.body.datasetId,pair:ds.rec.pair,timeframe:ds.rec.timeframe,params:req.body,result,createdAt:new Date().toISOString()};d.backtests.push(rec);save(d);res.json(rec)});
app.get('/api/backtests',(req,res)=>res.json(db().backtests.slice().reverse()));
...
app.post('/api/robot/export-mt5',(req,res)=>{
 try{
   const body=req.body||{};
...
app.post('/api/strategy/save',(req,res)=>{
 try{
   ...
   save(d);
   res.json({ok:true,strategy:rec,total:d.strategies.length});
 }catch(e){res.status(500).json({ok:false,error:e.message})}
});
app.get('/api/strategies',(req,res)=>{const d=db();res.json((Array.isArray(d.strategies)?d.strategies:[]).slice().reverse())});
...
function findLatestMt5AuditCsv(runId=''){
  const safeRun=String(runId||'').toLowerCase();
  ...
  if(safeRun){ const matched=files.filter(f=>f.name.toLowerCase().includes(safeRun)); if(matched.length) files=matched; }
...
function findLatestMt5Csv(runId=''){
  ...
  if(safeRun){
    const matched = files.filter(f=>f.name.toLowerCase().includes(safeRun));
...
function fileStateForImport(found){
  if(!found) return {ready:false, state:'missing', reason:'Nenhum CSV encontrado para este Run ID'};
...
function normalizeRobotConfig(robot, extra={}){
  ...
  const indicators = (vs.indicators||[]).map(x=>({type:String(x.type||'').toLowerCase(),period:Number(x.period||14)}));
  return {
    version:'53.0.0',
...
// =========================
// MQ5 VALIDATION REPLAY v92 - lógica idêntica ao Backtest Lab JS
// =========================
function buildValidationMq5V91(robot, cfg){
  const safe=mqlSafeName(cfg.name||robot?.name||'ForexIA_Validation');
  const robotId=String(cfg.robotId||robot?.id||'default');
  const inds=cfg.indicators||[];
  const get=(t,d)=>Number((inds.find(x=>String(x.type||'').toLowerCase()===t)||{}).period||d);
  const ema=get('ema',20), rsi=get('rsi',14);
  const startH=Number(String(cfg.filters?.startHour||'00:00').slice(0,2))||0;
  const endH=Number(String(cfg.filters?.endHour||'23:59').slice(0,2))||23;
  return `//+------------------------------------------------------------------+
//| ${safe}_VALIDATION_v92_JS_EXATO.mq5                              |
//| Forex IA v92 - Replay Engine igual ao Backtest Lab JS             |
//| Não abre ordens. Apenas gera CSV para validação.                  |
//+------------------------------------------------------------------+
#property strict
#property version   "91.00"
#property description "Forex IA Validation v92: replay sem ordens, formulas JS exatas, CSV comum."
...
int OnInit(){
   ...
   Print("FOREX IA v92 JS_EXATO iniciado. Sem ordens. RunId=", Validation_RunId, " Symbol=", _Symbol, " TF=", TF());
   return INIT_SUCCEEDED;
}
...
app.get('/api/robots/:id/validation-mq5',(req,res)=>{
  try{
    let r=findRobotCompat(req.params.id);
    if(!r){
    ...
    if(!r){
      ...
      r=findRobotCompat(req.params.id) || readProjects().find(x=>x.id===req.params.id || x.name===req.params.id);
    }
    ...
    const fileName=mqlSafeName(cfg.name||r.name||'ForexIA')+'_VALIDATION_v92_JS_EXATO.mq5'; saveRobotFolderMq5(r,code,fileName);
    res.setHeader('Content-Type','text/plain; charset=utf-8');
    res.setHeader('Content-Disposition',`attachment; filename="${fileName}"`);
    res.send(code);
  }catch(e){
