//+------------------------------------------------------------------+
//| ForexIA_Bridge_v25.mq5                                           |
//| MT5 Desktop -> Forex IA Studio v25 Backtest Lab                  |
//+------------------------------------------------------------------+
#property strict
...
void EnviarPacote(string symbol,string tfText,MqlRates &rates[],int inicio,int fim){
   ...
   string json="{\"bridge\":\"v25\",\"source\":\"MT5_BRIDGE_V25\",\"symbol\":\""+JsonEscape(symbol)+"\",\"timeframe\":\""+JsonEscape(tfText)+"\",\"candles\":[";
   ...
   json+="]}";
   if(PostJson("/api/mt5/candles",json)) Print("V25 OK: ",symbol," ",tfText," ",inicio," ate ",fim-1);
}
void EnviarHistorico(string symbol,string tfText,ENUM_TIMEFRAMES tf,bool completo){
   ...
   Print("V25 enviando: ",symbol," ",tfText," candles: ",copied);
...
int OnInit(){Print("FOREX IA BRIDGE V25 INICIADO"); EventSetTimer(AtualizarACadaSegundos); EnviarStatus("EA v25 iniciado"); if(EnviarHistoricoAoIniciar) EnviarTodos(true); return INIT_SUCCEEDED;}
void OnTimer(){EnviarTodos(false);}
void OnDeinit(const int reason){EventKillTimer(); EnviarStatus("EA v25 finalizado");}
